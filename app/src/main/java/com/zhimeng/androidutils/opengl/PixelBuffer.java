package com.zhimeng.androidutils.opengl;
...
public class PixelBuffer {
    ...
    private void convertToBitmap() {
        ...
        int[] pixels = ib.array();
        int[] line = new int[mWidth];

        for (int i = 0; i < mHeight / 2; i++) {
            System.arraycopy(pixels, i * mWidth, line, 0, mWidth);
            System.arraycopy(pixels, (mHeight - i - 1) * mWidth, pixels, i * mWidth, mWidth);
            System.arraycopy(line, 0, pixels, (mHeight - i - 1) * mWidth, mWidth);
        }

        mBitmap = Bitmap.createBitmap(mWidth, mHeight, Bitmap.Config.ARGB_8888);
        mBitmap.copyPixelsFromBuffer(IntBuffer.wrap(pixels));
    }
}
